...
current_generation_genomes = [spawner.spawn_genome(config) for _ in range(config.population_size)]
...
def incubate(config, genome, container, slot):
    print("%s: \t Spawning individual %i... " % (datetime.now().strftime('%X'), slot))
    dataset = rnn.LSTM_Dataset(folder_name = "C:/Users/Fredrik/Documents/WashedData/TurbinA", 
                        min_cycle_size = 2400, 
                        target_ranges = genome['target_ranges'], 
                        reuse_factor = 30, 
    ...
    with graph.as_default():
        network = rnn.LSTM_Network(input_size = dataset.input_dimension,
                                ...
                                num_steps = genome['window_size'],
                                batch_size = config.batch_size,
                                lstm_layers = genome['lstm_layers'],
                                ffwd_layers = genome['ffwd_layers'],
                                learning_rate = config.max_learning_rate)
    print('%s: \t Individual %i spawned.' % (datetime.now().strftime('%X'), slot))
    return graph, network

for generation in range(config.num_generations):
    print("%s: \t Generation %i" % (datetime.now().strftime('%X'), generation))
    generation_scores = np.zeros(config.population_size)
    current_generation_individuals = Array('' [None for _ in range(config.population_size)]
    for ind in range(len(current_generation_genomes)):
        p = Process(target=incubate, args=())
    ...
    for ind in range(len(current_generation_genomes)):
        ...
        for x in range(int(config.population_size/2)):
            ...
            parent0 = current_generation_genomes[parents[0]]
            parent1 = current_generation_genomes[parents[1]]
        ...
        current_generation_genomes = next_generation
